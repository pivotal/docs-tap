...
### Symptoms
  ...
  + The `convention-controller` [logs](#gathering-logs) reports an error `failed to get CABundle` like this.
    ```json
...
1. Ensure that the `convention server` ([ClusterPodConvention](reference/cluster-pod-convention.md)) is configured with the correct certificates. To do so, verify the value of annotation `conventions.apps.tanzu.vmware.com/inject-ca-from` which needs to be set to the used *Certificate*, Do not set annotation `conventions.apps.tanzu.vmware.com/inject-ca-from` , if no certificate is used.
...
### Symptoms
  + When a `PodIntent` is submitted, the `convention` is not applied.
  + The `convention-controller` [logs](#gathering-logs) reports a connection refused error like the following.
