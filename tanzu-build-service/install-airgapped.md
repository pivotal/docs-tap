...
Tanzu Build Service, and you are on an air-gapped environment.
...
1. Create a `tbs-values.yaml` file. The required fields for an air-gapped installation are as follows:
    ...
    - `REPOSITORY` is the fully qualified path to the Tanzu Build Service repository. <!-- does this mean the repo you want to install TBS in? "is a writable repository in your registry." >
...
You must install `full` dependencies.

To install the `full` dependencies:
