...
* If you are using `full` dependencies, you will need to complete a few extra steps to update your dependencies after you upgrade to the latest patch version. In this case, please see instructions on [Upgrading the full dependencies package](../upgrading.md#full-profile-upgrade-tbs-deps).
...
Using the dependencies packaged within a particular version of Tanzu Application Platform ensures that your Tanzu Build Service dependencies are compatible with the other components of Tanzu Application Platform. Therefore, we do not encourage updating dependencies outside of consuming new versions of Tanzu Application Platform. However, if you need to consume a new version of a buildpack or stack prior to the upcoming patch release of Tanzu Application Platform, you may do so using the `kpack` CLI. Waiting until the patch release ensures all dependencies are fully tested with Tanzu Application Platform, but there are some cases where users may want to update a dependency more quickly. If you are seeking to consume a newer stack version, please see our documentation on [updating a ClusterStack](https://docs.vmware.com/en/Tanzu-Build-Service/1.10/vmware-tanzu-build-service/updating-deps.html#cluster-stacks-update). If you are seeking to consume a newer buildpack version, see please see our documentation on [updating a ClusterStore](https://docs.vmware.com/en/Tanzu-Build-Service/1.10/vmware-tanzu-build-service/updating-deps.html#cluster-store-update). In either of these cases you will need the `kpack` CLI - if you do not have the `kpack` CLI installed, you can download it from [Tanzu Network](https://network.pivotal.io/products/build-service/). 
