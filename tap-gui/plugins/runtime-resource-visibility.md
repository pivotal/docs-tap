...
1. Define a Backstage component with a `backstage.io/kubernetes-label-selector` annotation. See
   ...
   - Use [Tanzu Application Platform Initializer](https://github.com/sample-accelerators/tap-initialize/blob/main/README.md) included in [Application Accelerator](application-accelerator.md) to generate the  `catalog-info.yaml` file. For additional instructions please refers to [Generate YAML files using Tanzu Application Platform Initializer](#-generate-yaml-files-using-tanzu-application-platform-initializer)
...
3. Create a Kubernetes resource with a label matching the component's selector in a cluster available to Tanzu Application Platform GUI. A resource is one of the following:
   ...
   - Use [Tanzu Application Platform Initializer](https://github.com/sample-accelerators/tap-initialize/blob/main/README.md) included in [Application Accelerator](application-accelerator.md) to generate the  `workload.yaml` file. For additional instructions please refers to [Generate YAML files using Tanzu Application Platform Initializer](#-generate-yaml-files-using-tanzu-application-platform-initializer)
...
This is a package available in [Application Accelerator](application-accelerator.md), you can visit its official page [here](https://github.com/sample-accelerators/tap-initialize/blob/main/README.md).
Steps to use it:

- Access the TAP Project Configuration in the Accelerators page or access it directly through Tanzu Application Platform GUI using `<TAP-GUI-URL>/create/templates/tap-initialize`.
- Fill the required information and click in **Next**.
  ![Screenshot of filling the blankets in the initializer](images/runtime-resources-initializer-configuration.png)
- Check the parameters to use the initializer and click in **Generate Accelerator**.
  ![Screenshot of confirming the parameters for the initializer](images/runtime-resources-initializer-confirmation.png)
- Once the process is complete you will be able to explore or download the package (a zip file) with the generated files, uncompress it and follow the `INSTRUCTIONS.md` file included in it.
