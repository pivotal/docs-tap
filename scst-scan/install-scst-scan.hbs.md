...
2. (Optional) Make changes to the default installation settings:
    ...
    If you are using Grype Scanner `v1.5.1 or later` or other supported scanners included with Tanzu Application Platform `v1.5.1 or later` and do not want to use the default SCST - Store integration, deactivate the integration by appending the following field to the `values.yaml` file:
