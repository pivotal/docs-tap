...
The `ScanTemplate` custom resource (CR) is where you define the Pod with the scanner image that you use for vulnerability scanning. There's a default scanner image you can use out-of-the-box.
...
1. **SourceScan**  
   - **Init Container**  
        If you're doing a `SourceScan`, we recommend defining the cloning of the repository in an init container named `repo`. Any output in `stdout` in this init container is prompted out if an error occurs, so you can have more context about what failed inside the job.
