...
1. Create a `ClusterImagePolicy` to specify the images that the webhook validates.
    ...
    - If no `ClusterImagePolicy` is created, all images will be permited into the cluster with the following warning: `Warning: clusterimagepolicies.signing.run.tanzu.vmware.com "image-policy" not found`.
