...
grype:
  ...
  metadataStore:
    ...
    authSecret: # (optional) Defaults all values to empty. Only for multi-cluster
...
- `METADATA-STORE-URL` is the base URL where the Supply Chain Security Tools - Store deployment can be reached, e.g.: `https://metadata-store-app.metadata-store.svc.cluster.local:8443`
- `CA-SECRET-NAME` is the name of the secret containing the ca.crt to connect to Supply Chain Security Tools - Store deployment.
- `SECRET-NAMESPACE` is the namespace where Supply Chain Security Tools - Store is deployed, if you are using a single cluster. If you are using multi-cluster, it is where the connection secrets were created.
- `TOKEN-SECRET-NAME` is the name of the secret containing the auth token to connect to Supply Chain Security Tools - Store deployment when installed in a different cluster, if you are using multi-cluster.
