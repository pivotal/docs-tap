...
You are aware<!--฿ To avoid anthropomorphism, use |detects|. ฿--> that the SLOs for each cluster are
...
simple<!--฿ Avoid suggesting an instruction is |simple| or |easy|. ฿--> Helm chart.
`run-test` has more robust requirements, so you'd<!--฿ Avoid a contraction if it is too colloquial or awkward or uncommonly used. ฿-->
...
They shouldn't<!--฿ Favour certainty, agency, and imperatives: |the app now works| over |the app should now work|. |VMware recommends| over |you should|. If an imperative, |do this| over |you should do this|. If |should| is unavoidable, it must be paired with information on the exceptions that |should| implies exist. ฿--> need to<!--฿ |must| is preferred or, better, rephrase as an imperative. ฿--> know about the inner workings of the services, and shouldn't<!--฿ Favour certainty, agency, and imperatives: |the app now works| over |the app should now work|. |VMware recommends| over |you should|. If an imperative, |do this| over |you should do this|. If |should| is unavoidable, it must be paired with information on the exceptions that |should| implies exist. ฿--> need to<!--฿ |must| is preferred or, better, rephrase as an imperative. ฿-->
...
- There are three clusters:<!--฿ Embolden run-in headings. ฿--> `iterate`, `run-test`, and `run-production`.
- In each cluster, the Service Operator creates a `ClusterInstanceClass` called postgres<!--฿ |PostgreSQL| is preferred. ฿-->.
  - In the `iterate` cluster, this is a provisioner-based class which uses the out of the box<!--฿ Do not use |out of the box| figuratively. ฿--> Bitnami services to provision helm <!--฿ |Helm| is preferred. ฿-->instances of postgres<!--฿ |PostgreSQL| is preferred. ฿-->.
  - In the `run-test` cluster, this is a provisioner-based class which uses VMware Tanzu Postgres<!--฿ |PostgreSQL| is preferred. ฿--> to provision instances of postgres<!--฿ |PostgreSQL| is preferred. ฿-->.
...
- The App Operator creates a `ClassClaim`, this gets<!--฿ There is likely a more precise and formal word to use here than |gets|. ฿--> applied along with<!--฿ Do not use as a conjunction. Whenever possible, use |with| or |and| instead. ฿--> a consuming `Workload`.
  - When it is applied in `iterate` it resolves to a helm chart<!--฿ |Helm chart| is preferred. ฿--> instance.
  - When it is promoted to `run-test` it resolves to a VMware Tanzu Postgres<!--฿ |PostgreSQL| is preferred. ฿--> instance.
  - When it is promoted in `run-production` it resolves to an Amazon AWS RDS instance.
- The definition of the `ClassClaim` remains identical across the clusters, thus<!--฿ Re-write the sentence to drop |thus| or, if that is not possible, replace with |therefore|. ฿--> making life nice and simple<!--฿ Avoid suggesting an instruction is |simple| or |easy|. ฿--> for the application development team
...
One other point worth mentioning is that although we<!--฿ |VMware|, the product name, or another term is preferred. Define who |we| is for the reader is preferred. ฿-->
are choosing to use provisioner-based classes on all three clusters here,
it's also possible to<!--฿ Active voice |you can also| might be better. ฿--> use a combination of
provisioner-based and pool-based classes across the clusters.
This may<!--฿ |can| usually works better. Use |might| to convey possibility. ฿--> be desirable in cases where, for example, you want to allow for
...
service instance.
This could<!--฿ |can| or |might| whenever possible is preferred. When providing examples, use simple present tense verbs instead of postulating what someone or something could or would do. ฿--> be achieved through use of a provisioner-based class on the `iterate` cluster
...
service instances.
To do that, you can refer to<!--฿ If telling the reader to read something else, use |see|. ฿--> the
...
You'll<!--฿ Avoid a contraction if it is too colloquial or awkward or uncommonly used. ฿--> need to<!--฿ |must| is preferred or, better, rephrase as an imperative. ฿--> complete
...
[Step 4: Create a Composition](../how-to-guides/dynamic-provisioning-tanzu-postgresql.hbs.md#create-composition) and
[Step 6: Configure RBAC](../how-to-guides/dynamic-provisioning-tanzu-postgresql.hbs.md#configure-rbac) only.
After you completed<!--฿ |you verbed| is punchier than |you've verbed| and it fits in most cases. ฿--> those steps, return back here to continue.
...
The `ClusterInstanceClass` will<!--฿ Avoid |will|: present tense is preferred. ฿--> act as the abstraction fronting the differing service implementations across the differing clusters.
We<!--฿ |VMware|, the product name, or another term is preferred. Define who |We| is for the reader is preferred. ฿-->'ll create a class with the same name on all three of the clusters,
but the configuration of the class will<!--฿ Avoid |will|: present tense is preferred. ฿--> vary slightly on each.
...
to<!--฿ Redundant? ฿--> power the pre-installed Postgres<!--฿ |PostgreSQL| is preferred. ฿--> service.
We<!--฿ |VMware|, the product name, or another term is preferred. Define who |We| is for the reader is preferred. ฿--> are simply<!--฿ Avoid suggesting an instruction is |simple| or |easy|. ฿--> reusing the underlying CompositeResourceDefinition here from a different class using the desired<!--฿ |that you want| is preferred. ฿--> class name.

Use `kubectl` <!--฿ Do not format |kubectl| as code. ฿-->to apply the file to the `iterate` cluster.
...
Use `kubectl` <!--฿ Do not format |kubectl| as code. ฿-->to apply the file to the `run-test` cluster.
...
Use `kubectl` <!--฿ Do not format |kubectl| as code. ฿-->to apply the file to the `run-production` cluster.
...
Now that all the setup is done, we<!--฿ |VMware|, the product name, or another term is preferred. Define who |we| is for the reader is preferred. ฿--> can switch roles to that of the Application Operator/Developer,
...
Then use `kubectl` <!--฿ Do not format |kubectl| as code. ฿-->to apply the file to the `iterate` cluster.
