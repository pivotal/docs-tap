...
The Out of the Box Templates package now ships with a Tekton `ClusterTask`
...
should be run from a Tekton `Pipeline`.
...
required packages.  You will need to set up one, but not both, of those
packages.

These supply chains will use the Jenkins service during the `source-tester`
phase of the pipeline.

### <a id="making-a-jenkins-test-job"></a> Making a Jenkins test job

The intent of the Jenkins task for the Out of the Box Templates is to help TAP
users keep their existing test suites on their Jenkins services and still
integrate with the modern application deployment pipeline that TAP provides.

This section of this guide has instructions on how to configure a Jenkins job to
be triggered by the TAP Jenkins task.

It is assumed that the Jenkins job is being used to run test suites on some
code.  In order for the Jenkins job to know which source code to test the
Jenkins task will call the Jenkins job with the following two parameters, even
if they are not declared in the `Workload` `job-params`.  The Jenkins task will
only pass these parameters, however, if they are defined in the Jenkins job
itself.

- `SOURCE_URL` **string** The URL of source code to be tested.  Note that this URL
...
- `SOURCE_REVISION` **string** The revision of the source code to be tested.
  The format of this value may vary depending on the implementation of the
  `source-provider` resource.  If the "source-provider" is the FluxCD
  `GitRepository` resource then the value of the `SOURCE_REVISION` will be the
  ...
  Jenkins pipeline script will have to extract the commit SHA from the
  `SOURCE_REVISION` in order to be useful.  See the example in the
  [Example Jenkins Job](#example-jenkins-job) for guidance.

If you are not able to use the `SOURCE_URL` (because your Jenkins service cannot
...
You will need to configure your `Workload` to pass the `GIT_URL` parameter into
the Jenkins task:
...
The `Workload` is described in the [Developer Workload](#developer-workload)
section below.
...
Assuming that you have saved the password to a file and you have saved the
...
- `source-url`, **required** An HTTP address where a `.tar.gz` file containing
  all the source code to be tested will be supplied.
- `source-revision`, **required** The revision of the commit or image reference
  found by the `source-provider`.
- `secret-name`, **required** The secret that contains the URL, username,
  password and certificate (optional) to the Jenkins instance that houses the
  job that needs to be run.
- `job-name`, **required** The name of the Jenkins job that needs to run.
...
- `jenkins-task`, **required** This `ClusterTask` should be one of the tasks that the
  pipeline should run in order to successfully trigger the Jenkins job.  It is
...
Save the above YAML definition to a file (e.g.: `pipeline.yaml`) then use the
...
Jenkins Adapter program in order to trigger the Jenkins job and wait for it to
complete. This container image is distributed with TAP on Tanzu Network but is
not installed at the same time as all the other packages; it is pulled at the
time that it is executed by a supply chain.  Hence, it does not implicitly have
...
Adapter container image.  If you have followed the TAP Install Guide then you
will have a `Secret` named `tap-registry` in each of your cluster's namespaces.
...
[service account](https://tekton.dev/docs/pipelines/pipelineruns/#specifying-custom-serviceaccount-credentials)
...
To enable the supply chain to run Jenkins tasks the `Workload` should include the
...
params:
  ...
  - name: testing_pipeline_matching_labels
    value:
      #! This label should match the label on the pipeline created above
      apps.tanzu.vmware.com/pipeline: jenkins-pipeline 
  ...
  - name: testing_pipeline_params
    value:
      ...
      #! can be empty. If empty, then set this value to `[]`.  If non-empty then the
...
that will sent to the Jenkins job.  The parameter are entered into the
`Workload` as a list of name-value pairs.  e.g.:
...
Jenkins job; you will have to set them in the `job-params` explicitly.
...
`SOURCE_REVISION` to determine which commit SHA to test, for example.  See [Making
a Jenkins Test Job](#making-a-jenkins-test-job) above for details on how to use
the git URL and source revision in a Jenkins test job.

Watch the quoting of the `job-params` value closely.  In the `tanzu apps
workload create` example above, the `job-params` value is a string with a JSON
structure in it.  The value of the `--param-yaml testing_pipeline_params`
parameter itself is a JSON string, hence the backslash escape characters before
