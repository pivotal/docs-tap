...
- `hostname`: Host name for the `networking.k8s.io/v1 Ingress`. If hostname is blank, no ingress
will be generated.
...
The git revision identifier for the software included to the Carvel Package will be added to the package version in the following format `+build.${git_commit}` in the following cases:
