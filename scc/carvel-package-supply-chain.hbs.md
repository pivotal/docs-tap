...
> `Deployment`. The Carvel `Package` wraps these resources. The `Ingress` is
> secured using the `cluster_issuer` `tap-ingress-selfsigned` by default.
...
`Server` workload type:
- `replicas`: Number of pods that you want for the `apps/v1 Deployment`. Default is `1`.
- `hostname`: Host name for the `networking.k8s.io/v1 Ingress`. If you don't need ingress, leave hostname as an empty string. If hostname is not included in the file, you see an error.
will be generated.
...
1. (Optional) Enable OpenAPIv3 Schema Definition Generation. Defaults to
  ...
  If the size of the resulting OpenAPIv3 specification exceeds a certain size, roughly 3KB, the Supply Chain does not function. See the [known issue](../release-notes.hbs.md#1-6-0-tap-gui-ki).
...
1. Install the [Out of the Box Basic Supply Chain](ootb-supply-chain-basic.hbs.md) package.
...
### <a id="create-workload"></a> Creating a Workload
...
By default, the Git revision identifier for the software included in the Carvel Package is added to the package version, in the format `+build.${git_commit}`, in the following cases:

  1. The `source-to-url-package` is used to build the package and the image building resource is either `kpack-template` or the `kaniko-template`. These templates will add the label git
  commit SHA of the source code used to the label `org.opencontainers.image.revision` on the OCI image container.

  1. an OCI image container is used with the label `org.opencontainers.image.revision` already set

The package version may look similar to `20230518150903.0.0+build.6db88c7` where `6db88c7` is the git commit SHA.
