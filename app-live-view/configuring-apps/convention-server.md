...
The convention recognizes PodIntents for running Spring Boot apps and adds the following metadata labels:
...
| `tanzu.app.live.view.application.flavours` | `spring-boot` | Label | When deploying a Workload in Tanzu Application Platform, this label is set to `spring-boot` as default across the supply chain. |
...
in the Spring Boot Application. Properties defined in this way have lower priority and will be overridden
