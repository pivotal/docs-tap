...
## <a id="troubleshoot"></a>Troubleshoot Tanzu GitOps Reference Implementation (RI)
...
This indicates that the resource `packageinstall/tap` in namespace `tap-install` has failed. 
...
The resource to check is the TAP package install in the `tap-install` namespace:
