...
1. (Optional) Make changes to the default installation settings:
    ...
    1. Create a `cnr-values.yaml` using the following sample as a guide:
        ...
        Cloud Native Runtimes reuses the existing `tanzu-system-ingress` Contour installation for external and internal access when installed in the `dev` or `full` profile. 
        If you wish to use a separate Contour installation for system-internal traffic, set `cnrs.ingress.internal.namespace` to the empty string (`""`).
...
with your relevant values. The meanings of some placeholders are explained in this example:
    ...
    - `EXTERNAL-IP:PORT` is your Ingress host name or LoadBalancer information.
