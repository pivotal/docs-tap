...
If you configure the FLux or TAP Source COntrollers to use TLS and use custom certificates,
then you can configure the Accelerator System to skip the TLS verification for calls to access the
sources by providing the following property in the `accelerator` section of the `tap-values.yaml` file:
...
In order to enable TLS for the Accelerator Server, the following properties should be provided in
