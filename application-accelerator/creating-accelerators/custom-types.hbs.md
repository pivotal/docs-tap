...
The notation also provides the composition operators `merge` and `chain`, which enables
...
patterns. This ensures that the accelerator only detects files in the repository that match the
list of patterns.
...
as options, but instead of being supplied from a UI widget, they are computed by the accelerator
itself.
