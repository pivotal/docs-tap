...
When requesting a workshop using ``WorkshopRequest``, a login prompt for the workshop instance is presented to a user when accessing the URL for the workshop instance. By default the username is ``learningcenter``. The password is a random value which the user needs to query from the ``WorkshopRequest`` status after the custom resource has been created.
